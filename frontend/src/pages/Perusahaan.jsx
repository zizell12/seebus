...
import { Search, MapPin, Phone, Mail, ShieldCheck } from 'lucide-react'
import backgroundPerusahaan from '../assets/background-db.png'
import { Link } from 'react-router-dom'
import { useLanguage } from '../context/LanguageContext'
function MapPreview({ lat, lng, zoom = 15, label, alt }) {
  const staticMapUrl = `https://staticmap.openstreetmap.de/staticmap.php?center=${lat},${lng}&zoom=${zoom}&size=600x300&markers=${lat},${lng},red-pushpin`
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 md:px-6 py-14">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch mb-8">
        <div>
          <h2 className="text-2xl font-bold text-navy-900 mb-4">{t.perusahaan.misiJudul}</h2>
          <p className="text-gray-600 italic mb-6">{t.perusahaan.misiKutipan}</p>
          <div className="flex items-center gap-3">
            <img
              src="/images/avatar-placeholder.jpg"
              alt="Lala Vanisa Putri"
              className="w-10 h-10 rounded-full object-cover"
            />
            <div>
              <p className="text-sm font-semibold text-navy-900">Lala Vanisa Putri</p>
        ...
        <div className="bg-navy-900 rounded-xl p-8 flex flex-col justify-center">
          ...
          <h3 className="text-white font-bold text-lg mb-2">{t.perusahaan.perjalananAmanJudul}</h3>
          <p className="text-white/70 text-sm">{t.perusahaan.perjalananAmanDeskripsi}</p>
        </div>
      </div>

      <div className="grid grid-cols-3 gap-4">
        <StatItem value="500+" label={t.perusahaan.statArmada} />
        <StatItem value="120" label={t.perusahaan.statRute} />
        <StatItem value="2Juta+" label={t.perusahaan.statPenumpang} />
      </div>
    </section>
  )
}
function CommitmentBanner({ t }) {
  return (
    <section className="bg-navy-900 rounded-xl mx-4 md:mx-8 my-14 px-6 md:px-12 py-14 text-center">
      <h2 className="text-white text-2xl md:text-3xl font-bold mb-3">{t.perusahaan.komitmenJudul}</h2>
      <p className="text-white/70 max-w-xl mx-auto mb-6 text-sm">{t.perusahaan.komitmenDeskripsi}</p>
      <Link
  ...
  return (
    <div>
      <section
        ...
        style={{
          backgroundImage: `linear-gradient(to right, rgba(255,255,255,0.95), rgba(255,255,255,0.4)), url(${backgroundPerusahaan})`,
        }}
      >
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-16 md:py-24">
          <div className="max-w-xl">
            ...
            <p className="text-gray-600 mb-6 text-sm md:text-base">{t.perusahaan.heroDeskripsi}</p>
            <Link
              to="/"
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-6 py-14 grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
        <div>
          <h2 className="text-2xl font-bold text-navy-900 mb-6">{t.perusahaan.hubungiKamiJudul}</h2>
          <div className="space-y-5">
            ...
            />
            <ContactItem icon={Phone} label={t.perusahaan.labelPhone} value="+62 21 555 0192" />
            <ContactItem icon={Mail} label={t.perusahaan.labelEmail} value="support@seebus.co.id" />
          </div>
        </div>
        <MapPreview lat={-6.2241} lng={106.8022} label={t.perusahaan.labelPeta} alt={t.perusahaan.altPeta} />
      </section>
